...
void MainWindow::on_actionDateiNeu_triggered()
{
    ...
    tt.add(t, name, tmpur);
    tt.get_prgtext()->set_maschinengeometrie(lade_maschinendarstellung());
    update_gui();
...
text_zeilenweise MainWindow::lade_maschinendarstellung()
{
    QFile file(QDir::homePath() + QDir::separator() + CAD_Maschine);
    text_zeilenweise tzmaschine;
    if (file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        while(!file.atEnd())
        {
            ...
            if(tzmaschine.zeilenanzahl() == 0)
            {
                tzmaschine.set_text(line);
            }else
            {
                tzmaschine.zeilen_anhaengen(line);
            }
        }
        tzmaschine = kompatiblitaetspruefung(tzmaschine);
        file.close();
...
void MainWindow::on_actionMaschinengeometrie_bearbeiten_triggered()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        undo_redo tmpur;
        tmpur.set_groesse_max(settings_anz_undo_t.toInt());
        tt.add(t, QDir::homePath() + QDir::separator() + CAD_Maschine, tmpur);
        file.close();
        aktualisiere_anzeigetext();
